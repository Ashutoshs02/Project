...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES['party_logo']['name']) {
        $logo_name = basename($_FILES['party_logo']['name']); // Store only filename
        $target_path = "uploads/" . $logo_name;

        if (move_uploaded_file($_FILES['party_logo']['tmp_name'], $target_path)) {
            if ($old_logo && file_exists("uploads/" . $old_logo)) {
                unlink("uploads/" . $old_logo); // Delete old logo
            }
        } else {
    ...
    $update_query = "UPDATE candidates SET 
                     ...
                     party_logo='$logo_name' 
                     WHERE id=$id";

    if ($conn->query($update_query) === TRUE) {
        echo "<script>alert('Candidate details updated successfully!'); window.location.href='manage_candidate.php';</script>";
    } else {
        echo "<script>alert('Error updating candidate details!');</script>";
    }
}
...
<main class="container mt-5" style="padding-top: 100px; margin-bottom: 120px;">
    <div class="container d-flex justify-content-center">
        <div class="border p-4 shadow-lg rounded bg-light" style="width: 50%; max-width: 500px;">
            <h2 class="text-center mb-4 bg-primary text-white py-2 rounded">EDIT &nbsp; CANDIDATE &nbsp; DETAILS</h2>
            <form method="POST" enctype="multipart/form-data">
                <input type="hidden" name="old_logo" value="<?= $candidate['party_logo'] ?>">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="mobile_no" class="form-control" value="<?= $candidate['mobile_no'] ?>" required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="party_name" class="form-control" value="<?= $candidate['party_name'] ?>" required>
                </div>

                <div class="mb-3">
                    <label class="form-label">Party Logo:</label><br>
                    <img src="uploads/<?= $candidate['party_logo'] ?>" width="100" class="mb-2"><br>
