...
// Fetch all candidates from MySQL
...
<main class="container mt-5" style="padding-top: 100px; margin-bottom: 120px;">
    ...
    <tbody>
        ...
        if ($result->num_rows > 0) {
            $count = 1; // Counter for ID Column
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        <td>{$count}</td> <!-- Display ID Column -->
                        <td><img src='uploads/{$row['party_logo']}' width='60' height='60' class='rounded'></td>
                        ...
                        <td>{$row['gender']}</td>
                        <td>
                            <a href='edit_candidate.php?id={$row['id']}' class='btn btn-warning btn-sm me-2'>EDIT</a>
                            <a href='delete_candidate.php?id={$row['id']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Are you sure?\")'>DELETE</a>
                        </td>
                      </tr>";
